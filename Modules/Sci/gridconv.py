...
def grid_refinement(phi,n,v,ax0):
	# make sure r > 1
	...
	while 1:
		...
		if math.isnan(p[-1]) or math.isinf(p[-1]):
			print a,b,c,d,e
	...
	yo = round(math.log(max(phi) - min(phi), 10),0)
	yo = yo - 1
	y = pow(10,yo)
	y2 = round(min(phi)-y,-yo)
	y3 = round(max(phi)+y,-yo)
	
	print yo,y,y2
	
	ax.get_xaxis().set_major_formatter(matplotlib.ticker.LogFormatter(10,False))
	ax.get_xaxis().set_minor_formatter(matplotlib.ticker.LogFormatter(10,False))

	matplotlib.ticker.LogFormatter(10,True)
	
	#ax.axis([h[0]/1.2,h[-1]*1.2,y2,y3])
	ax.axis([h[0]/1.2,h[-1]*1.2,0,y3])
